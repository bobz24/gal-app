...
if (isset($_POST['action']) && $_POST['action'] == "status_post"){
	...
	if(strlen($_POST['data']) < 1){
	   ...
	   exit();
        ...
        $sql = "SELECT COUNT(id) FROM members WHERE user='$account_name' AND activated='1' LIMIT 1";
        ...
        mysqli_query($db_con, "UPDATE status SET osid='$id' WHERE id='$id' LIMIT 1");
        ...
        if ($row[0] > 9) { // If they have 10 or more posts of type a
            ...
            $row = mysqli_fetch_row($query);
            $oldest = $row[0];
            mysqli_query($db_con, "DELETE FROM status WHERE osid='$oldest'");
        }
        // Insert notifications to all friends of the post author
        $friends = array();
        $query = mysqli_query($db_con, "SELECT user/*, friend*/ FROM friends WHERE friend='$log_username' AND accepted='1'");
        while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)){
			 array_push($friends, $row["user"]);
			 //array_push($friends, $row["friend"]);
			 //print_r($friends);
             }
             //additional code for correcting friends array bug
             $query = mysqli_query($db_con, "SELECT friend FROM friends WHERE user='$log_username' AND accepted='1'");
        while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)){
             array_push($friends, $row["friend"]);
		}
        //$query = mysqli_query($db_con, "SELECT user FROM friends WHERE friend='$log_username' AND accepted='1'");
        //while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) { array_push($friends, $row["friend"]); }
        //$test = print_r($friends);
        for($i = 0; $i < count($friends); $i++){
            $friend = $friends[$i];
            $app = "Status Post";
            $note = $log_username.' posted on: <br /><a href="member.php?u='.$account_name.'#status_'.$id.'">'.$account_name.'&#39;s Profile</a>';
            mysqli_query($db_con, "INSERT INTO notifications(username, initiator, app, note, date_time) VALUES('$friend','$log_username','$app','$note',now())"); 
        }
        mysqli_close($db_con);
        echo "post_ok|$id";
        exit();
...
// Make sure data is not empty
    ...
    while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
        ...
        $app = "Status Reply";
        $note = $log_username.' commented here:<br /><a href="member.php?u='.$account_name.'#status_'.$osid.'">Click here to view the conversation</a>';
        mysqli_query($db_con, "INSERT INTO notifications(username, initiator, app, note, date_time) 
...
if (isset($_POST['action']) && $_POST['action'] == "delete_status"){
    ...
    $query = mysqli_query($db_con, "SELECT account_name, author FROM status WHERE id='$statusid' LIMIT 1");
    while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
        $account_name = $row["account_name"]; 
        $author = $row["author"];
    }
    if ($author == $log_username || $account_name == $log_username) {
        mysqli_query($db_con, "DELETE FROM status WHERE osid='$statusid'");
        mysqli_close($db_con);
        echo "delete_ok";
        exit();
    }
...
if (isset($_POST['action']) && $_POST['action'] == "delete_reply"){
    ...
    $query = mysqli_query($db_con, "SELECT osid, account_name, author FROM status WHERE id='$replyid' LIMIT 1");
    while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
    $osid = $row["osid"];
    $account_name = $row["account_name"];
    $author = $row["author"];
    ...
    if ($author == $log_username || $account_name == $log_username) {
        mysqli_query($db_con, "DELETE FROM status WHERE id='$replyid'");
        mysqli_close($db_con);
        echo "delete_ok";
    exit();
    }
